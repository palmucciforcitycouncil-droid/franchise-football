...
PARAMS: Dict[str, Any] = {
    ...
    # (7 types, not the dozen-plus real NFL penalties), and there's no
    ...
    # per-type rolls; their sum (~2.8%) is close to the old flat 3%.
    "penalty": {
        "pre_snap": {
            "false_start": 0.014,
            "delay_of_game": 0.003,
            "illegal_formation": 0.003,
            "offside": 0.009,
        },
        "in_play": {
            "offensive_holding": 0.045,             # per run play only
            "defensive_pass_interference": 0.022,   # per incomplete pass only
            "roughing_the_passer": 0.05,             # per sack only
        },
        "dpi_yards": 12,
        "holding_yards": 10,
        "roughing_yards": 15,
