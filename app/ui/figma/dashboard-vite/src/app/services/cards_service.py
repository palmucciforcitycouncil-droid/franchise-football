...
from app.services.cap_service import cap_snapshot_for_release, cap_snapshot_for_resign
...
def build_player_card(sess: Session, player_id: int, season: int) -> Dict[str, Any]:
    ...
    # Accolades
    accolades = player_accolades(sess, player_id)

    # Cap previews
    cap_release = cap_snapshot_for_release(sess, player_id, season)
    # If there is an ask, show resign projection vs current hit using the ASK aav
    cap_resign = {}
    if ask:
        cap_resign = cap_snapshot_for_resign(sess, player_id, getattr(ask, "desired_aav", 0), season)

    # Recent games mini-log
    recent = recent_player_games(sess, player_id, season, limit=3)

    card["accolades"] = accolades
    card["cap"] = {"release": cap_release, "resign_projection": cap_resign}
    card["recent_games"] = recent
    ...
    card = {
        ...
        "accolades": accolades,
        "cap": {"release": cap_release, "resign_projection": cap_resign},
        "recent_games": recent,
        "actions": actions
    }
...
def build_coach_card(sess: Session, coach_id: int, season: int) -> Dict[str, Any]:
    ...
    accolades = coach_accolades(sess, coach_id)
    recent = recent_coach_games(sess, coach_id, season, limit=3)
