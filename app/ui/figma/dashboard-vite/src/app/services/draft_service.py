...
from app.models.draft import Prospect, ProspectPosition, ScoutingReport, DraftBoard, DraftPickInventory, DraftState
...
def generate_draft_class(sess: Session, season: int, seed: int = 4242, size: int = 240) -> int:
    ...
    for rnd in range(1,8):
        for slot in range(1,33):
            ...
            sess.add(DraftPickInventoryInventory(season=season, round=rnd, slot=slot, owning_team_id=slot, original_team_id=slot))
...
def _materialize_player_from_prospect(sess: Session, prospect_id: int, team_id: int, season: int, round:int, slot:int):
    ...
    # Compliance checks
    if not roster_has_room(sess, team_id):
        raise ValueError("Roster full (53); cannot sign rookie")
    if not can_afford(sess, season, team_id, rc["aav"]):
        raise ValueError("Cap space insufficient for rookie deal")
...
def pick_on_clock(sess: Session, season: int, team_id: int, prospect_id: int) -> dict:
    ...
    # validate owner
    pk = sess.exec(select(DraftPickInventory).where(DraftPickInventory.season==season, DraftPickInventory.round==st.current_round, DraftPickInventory.slot==st.current_pick_slot)).first()
