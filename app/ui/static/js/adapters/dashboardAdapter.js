/* dashboardAdapter v1.1 — tries real API, falls back to demo */
(function(){
  ...
  window.uiAdapters.dashboard = {
    ...
    async startPlayoffs(){ try{ const r=await fetch('/api/season/playoffs',{method:'POST'}); if(!r.ok) throw new Error(); return await r.json(); } catch(e){ console.warn(LOG,'playoffs fallback',e); return {ok:true,demo:true}; } },
    async processOffseason(){ try{ const r=await fetch('/api/season/offseason',{method:'POST'}); if(!r.ok) throw new Error(); return await r.json(); } catch(e){ console.warn(LOG,'offseason fallback',e); return {ok:true,demo:true}; } },
  };
