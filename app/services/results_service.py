...
def upsert_team_game(sess: Session, payload: dict):
    ...
    """
    row = sess.exec(select(TeamGameStats).where(TeamGameStats.game_id==payload["game_id"], TeamGameStats.team_id==payload["team_id"])).first()
    if not row:
...
def bulk_upsert_player_boxes(sess: Session, boxes: List[dict]):
    ...
    """
    for b in boxes:
        row = sess.exec(select(PlayerBox).where(PlayerBox.game_id==b["game_id"], PlayerBox.player_id==b["player_id"])).first()
        if not row:
            row = PlayerBox(**b)
        else:
            for k,v in b.items(): setattr(row, k, v)
        sess.add(row)
    sess.commit()
...
def game_box(sess: Session, game_id: int) -> Dict[str,Any]:
    """Get the complete box score for a game."""
    # Team lines
    ...
    return {
        "teams": [ {k:getattr(t,k) for k in TeamGameStats.__fields__.keys() if k!="id"} for t in teams ],
        "players": [ {k:getattr(p,k) for k in PlayerBox.__fields__.keys() if k!="id"} for p in players ]
    }
