#!/usr/bin/env python3
"""
ULTIMATE HFA TEST - Final Home Field Advantage Tuning
Tests the ultimate HFA tuning to achieve realistic 52-54% home win rate.
"""
...
os.environ['HFA_POINTS'] = '2.0'  # ULTIMATE HFA tuning
os.environ['LEAGUE_SEED'] = '2025'

def ultimate_hfa_test():
    """Run ultimate HFA test targeting realistic home win rate"""
    
    def _yardage_targets_v2_ultimate(season: int, home_id: int, away_id: int) -> Tuple[int, int, int, int]:
        """Generate realistic yardage targets with ultimate HFA."""
    ...
    def _points_from_yards_v2_ultimate(home_abbr: str, away_abbr: str) -> Tuple[int, int]:
        """Convert yardage to realistic scores with ULTIMATE HFA tuning."""
        base_seed = int(os.environ.get("LEAGUE_SEED", "2025"))
        ...
        # Generate yardage
        home_total, home_pass, away_total, away_pass = _yardage_targets_v2_ultimate(2025, 1, 2)
        ...
        # ULTIMATE HFA: Single +2.0 points (no double HFA)
        hfa_points = float(os.environ.get("HFA_POINTS", "2.0"))
    ...
    print("=== ULTIMATE HFA TEST - FINAL HOME FIELD ADVANTAGE TUNING ===")
    print("Targeting 52-54% Home Win Rate (Realistic NFL)")
    print("ULTIMATE HFA: Single +2.0 points (no double HFA)")
    ...
    for season in range(2025, 2028):
        ...
        for game in range(272):
            ...
            h_total, h_pass, a_total, a_pass = _yardage_targets_v2_ultimate(season, home_id, away_id)
            h_score, a_score = _points_from_yards_v2_ultimate(f"TEAM{home_id}", f"TEAM{away_id}")
    ...
    print("="*60)
    print("=== ULTIMATE HFA THREE-SEASON SUMMARY (816 GAMES) ===")
    ...
    if targets_met >= 4:
        print("SUCCESS: ULTIMATE HFA TEST SUCCESSFUL!")
    else:
