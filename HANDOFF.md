...
1. **Season-week game inspection** — now that single games have a real play-by-play view (item 8), the natural follow-on is making a season week's individual games inspectable the same way; needs `season_state.py` to retain each week's `GameResult`s (or at least the play lists), not just final scores.
