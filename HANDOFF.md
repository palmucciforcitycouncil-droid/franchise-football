...
- **No per-play UI** — `PlayEvent.defensive_call` (and play-by-play generally) isn't rendered anywhere; `result.html` only shows drive-level summaries and team totals. A per-play table would be the natural next step to make the new defensive calls (or the existing offensive play-by-play) visible.
...
1. **Per-play UI / play-by-play page** — the defensive AI (and the offensive play-calling before it) now produce a real `defensive_call` description per play, but nothing renders it; a play-by-play table would make both visible for the first time.
...
3. **Score Fidelity System** (GDD §6.2) — real calibration against target scoring distributions. Note: adding the defensive AI already moved combined scoring from ~63-64 toward ~48-55 as a side effect, not a deliberate tune — worth re-measuring before doing real calibration work here, since the starting point just moved.
