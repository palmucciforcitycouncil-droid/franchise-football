#!/usr/bin/env python3
"""
FINAL WORKING three-season simulation with PROPERLY WORKING home field advantage.
"""
...
def final_working_test():
    """Run FINAL WORKING three-season simulation with PROPERLY WORKING home field advantage"""
    ...
    def _points_from_yards_v2_working(home_abbr: str, away_abbr: str) -> Tuple[int, int]:
        """Convert yardage to realistic scores with WORKING home field advantage."""
        base_seed = int(os.environ.get("LEAGUE_SEED", "2025"))
        
        # Use different seeds for home and away teams to ensure different distributions
        ...
        # Generate yardage with different seeds
        home_total = max(220, min(550, int(r_home.normalvariate(340, 55))))
        away_total = max(220, min(550, int(r_away.normalvariate(340, 55))))
        ...
        # WORKING: Apply home-field advantage properly
        hfa_points = float(os.environ.get("HFA_POINTS", "1.6"))
        
        # Home team gets the advantage
    ...
    print("=== FINAL WORKING THREE-SEASON SIMULATION ===")
    print("Testing 272 games per season (NFL standard) for seasons 2025-2027")
    print("With FINAL WORKING home field advantage implementation")
    print()
    ...
    for season in range(2025, 2028):
        ...
        for game in range(272):
            ...
            h_score, a_score = _points_from_yards_v2_working(f"TEAM{home_id}", f"TEAM{away_id}")
