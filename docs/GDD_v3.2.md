...
\*Implementation status (2026-09-12, R3d -- full account in ROADMAP.md \\\S4c/\\\S4d and \`docs/R3d\_COACHING\_SYSTEM\_SPECIFICATION.md\`): \*\*the full hiring/firing/promotion market is built.\*\* The literal JSS formula below (line "JSS = 0.55×WinPct + ...") is the \*\*original\*\* spec text and is superseded for HC/OC/DC/ST by R3d's Enhanced JSS (context-aware: WinPerformance 25%, PerformanceVsExpectation 30%, PlayoffPerformance 15%, Trajectory 10%, Blowouts/Dysfunction 10%, Owner/Organizational 10% -- see the spec doc Sec 3.1 for the full per-role breakdown and every disclosed real-data substitution); the OLD formula (\`coach\_records.job\_security\_score()\`) is kept only because an existing test asserts its literal math, and is otherwise dead code. \*\*Firing is probabilistic, not a hard threshold\*\*: \`app/engine/coach\_hiring.firing\_probability()\` runs a logistic model over JSS, gated by real in-season week/tenure/recent-success modifiers, seeded so a replay is identical. \*\*OwnerWinPressure\*\* (\`app/services/owner\_pressure\_store.py\`) replaces the flat OwnerPatience stub with a real, persistent, franchise-level (not per-coach) value that adjusts once a season and carries decaying recent-success buffers. \*\*Replacement logic\*\* (\`app/engine/coach\_replacement.py\`) picks internal promotion (real InterimPromotionScore, real title-based promotion hierarchy) vs. an external hire (real HiringMerit + CandidateInterest) from a real coach pool (\`app/services/coach\_pool.py\`): Tier 2 is exactly the real fired/unemployed 433-seed coaches; Tier 3 is 100 real named college/former-NFL candidates seeded from Brian's own \`data/raw/coaches/Top\_100\_Football\_Coaching\_Candidates.md\` (the Excel file this section's spec write-up assumed never existed -- this real substitute does, with generated ratings inside the spec's own tiered bands plus a disclosed keyword-based credential bonus off each candidate's real background text). \*\*All 31 AI teams run this autonomously\*\* every offseason and (lightly) in-season; the user's own team gets real Fire/Hire buttons on the Staff page. \*\*Disclosed scope cuts\*\*: no multi-team AI bidding wars, no coach salary negotiation, no coaching salary cap (matches R4's own precedent for players) -- see the spec doc Sec 15 for the full list. \*\*Not done\*\*: PreseasonPowerRankDelta (below) is superseded outright by the Enhanced JSS's own real PerformanceVsExpectation term (\`app/services/team\_expectations.py\`), so it was never separately wired -- not an open gap, a different real mechanism replacing it.\*

\*Roster Strength (A1/A2, added 2026-09-11):\* \`app/engine/roster\_strength.py\` is the real roster-to-team-strength aggregator this section's PreseasonPowerRankDelta (and Sec 8.3.3's Team\_Quality\_Points, and team prestige generally) need and never had. Real player attributes and the real depth chart (snap-share-weighted per position, reusing app/engine/rotation.py's actual playing-time curves) roll up into 14 position-group ratings, a weighted \`roster\_score\`, and a \`team\_rating\` that additionally blends in the team's real Head Coach \`overall\` (15% weight -- "heavy," comparable to QB's own share, without one number outweighing ~40 real rostered players). The 14 positional weights are a documented first-pass placeholder, not a tuned result -- see ROADMAP.md's A1 entry for the full accounting and the required follow-up tuning pass. Its first real consumer is live: the Dashboard's Power Rankings box now also renders an FBGM-style position-rank sheet (Team/Conf/Div/Team Rating/Avg Age + a 1-32 rank per position group + Coach, sortable, horizontal-scrolling) computed fresh on every page load rather than from a stored snapshot -- see ROADMAP.md's A2 entry for why that's equivalent, not a shortcut, given nothing in this engine changes a player's or coach's rating mid-season yet. \*\*Still not wired\*\*: PreseasonPowerRankDelta above still computes as 0 -- JSS's own consumption of this rating needs a separate scaling/normalization decision not yet made, tracked as the explicit next task after A2 in ROADMAP.md.
...
\*Implemented 2026-09-12 (ROADMAP.md R4a, full account there): real cap math and a real, disclosed-simplified negotiation (\`app/engine/contracts.py\`, \`/gm-desk\`). Sec 8.3.1's tiered-CSV Market Anchor is replaced by a formula reusing \`roster\_strength.POSITION\_WEIGHTS\` (A1/A3's real, tuned number) instead of uncalibratable data this project doesn't have. Sec 8.3.3's Mood Meter (stateful, multi-round patience) is NOT built -- a single deterministic ACCEPT/REJECT/COUNTER verdict per offer instead. No rookie scale (needs the Draft, R5, which needs this chunk first). No signing-bonus cap proration/dead-cap -- cap hit = salary (AAV) only. \*\*Neither the \\$301.2M real cap figure above nor its stale \\$279.2M predecessor is what this engine actually uses for the dollar amount\*\* -- confirmed against the real imported Madden salary data, that number made every real team permanently over cap (real team totals run \\$262M-\\$695M; a single real player, Mahomes, is \\$190.4M alone -- a different scale than any real-world NFL cap, current or prior). \`SALARY\_CAP\_BASE\` is rescaled to \\$720M in code, disclosed in \`contracts.py\`'s own module docstring, not silently changed. \*\*The +7.5% annual GROWTH RATE above IS what this engine uses\*\* (updated 2026-09-12 from +11.2%) -- a rate has no scale-incompatibility, so \`SALARY\_CAP\_GROWTH\` tracks the real-world number directly, unlike the base dollar figure.\*
...
\- \*\*Initial Cap (2026, updated 2026-09-12 from the stale 2025 figure below):\*\* \*\*\\$301.2M\*\*

\- \*\*Annual Growth (updated 2026-09-12):\*\* \*\*+7.5%\*\* (compounded annually)
...
\*\*Amended 2026-09-12 (Brian's explicit decision, ROADMAP.md \\S4e has the full scope):\*\* headlines will be generated by a deterministic template bank instead of a live Claude API call. The event-detection/tiering/scoring substance above (12.2/12.3) is unchanged and still fully real -- only the LAST step (turning a detected event into a sentence) changes, from an LLM call to picking a phrasing variant via this project's own `stable_seed(LEAGUE_SEED, ...)` convention (\\S1.3) and filling in real stat values. Reasoning: this is the only feature in the entire project that would otherwise call an external API, which conflicts with \\S1.3's Determinism \& Seeding Policy ("given the same LEAGUE\\_SEED... results must be perfectly reproducible") and would need new infrastructure this project has none of today (API key/secrets config, a network-failure fallback path, generated-text caching so a reload doesn't reword last week's headline). A template bank needs none of that and produces the same real, notability-ranked content. No internet-fallback message is needed since there's no network call to fail.
...
\*\*Filled in 2026-09-12 (ROADMAP.md \\S4e has the full build scope, including a real, disclosed deviation from 12.1's "generated via API call to Claude" -- see 12.1's own amendment note below).\*\*



\*\*Cross-tier selection order\*\* is already fully specified by 12.2.1/12.2.2/12.2.3's own text -- a strict waterfall, not a blended cross-tier score: all present Tier 1 events first (capped at 3), then Tier 2 events fill any remaining slots up to the 4-5 target, then Tier 3 fills whatever's still short. This section only needed to supply what 12.2.3 called "rank within tier by secondary criteria" -- i.e. a real magnitude score for ordering WITHIN a tier, and a deterministic tie-break when magnitude ties.



\*\*Within-tier magnitude score (0-20, added to a fixed tier base of Tier1=100/Tier2=50/Tier3=20 purely so tiers never invert):\*\*



\- \*\*1c blowouts:\*\* `min(20, (margin - 21) / 2)` -- a 21-point win scores 0 bonus, a 41+ point win caps at 20.

\- \*\*1c upsets:\*\* `min(20, (power_rating_gap - 25) / 2)`, same shape.

\- \*\*2a stat milestones:\*\* `min(20, (stat_value - threshold) / threshold * 20)` -- e.g. 412 pass yards vs. the 300 threshold scores partway up the range.

\- \*\*2a your-team performance:\*\* `min(20, (power_rating_gap - 15) / 2)` for upset win/loss; `min(20, (points - 35))` for 35+; `min(20, (10 - points_allowed) * 2)` for the shutout/near-shutout side.

\- \*\*3a injuries:\*\* `min(20, overall_rating - 80)` if a rated starter, else a flat 10 for any other real depth-chart starter.

\- \*\*3b close games:\*\* `min(20, (3 - margin) * 6)` for a <=3-point final margin. The halftime-comeback half of 3b's condition ("trails by 14+ at any half and wins") is \*\*not buildable\*\* -- this engine has no quarter/clock model anywhere (same disclosed gap the Dashboard's Box Score box and \\S6.5 already carry), so there is no `score_by_quarter` to test a halftime deficit against. 3b is scoped as final-margin-only until a clock model exists.

\- \*\*3c streaks:\*\* `min(20, (streak_length - threshold) * 4)` off whichever round-number threshold (5/7/10) the streak just cleared.

\- \*\*"Your team" bonus:\*\* +10 flat, added on top of the above, to any event involving the user's own team -- matching 1a/2a's own "Your Team" framing in the tier text, so a tied score still surfaces the user's own storyline first.



\*\*Deterministic tie-break\*\* (magnitude score still equal after the above): order by `stable_seed(LEAGUE_SEED, season_number, week, event_key)` per \\S1.3's Determinism \& Seeding Policy, the same reproducibility convention every other tie-break/random draw in this engine already uses -- never an unordered dict/set iteration.
...
**Root cause, confirmed against the real DB (`data/franchise_football.db`, 433 real-seeded coaches, read-only investigation):** `Coach.overall` blends `reputation` (Sec S.2/7.7.2.3) with six performance ratings drawn centered on it. `reputation` came from `reputation_from_salary()` (`scripts/import_coaches.py`), a salary PERCENTILE computed independently within each role tier (HC / OC+DC+ST / AC) and mapped onto the SAME shared 40-99 band for every tier. Because every tier's highest earner lands near the 99th percentile of their own tier regardless of real dollars, the real data showed HC median `overall` **73** vs. AC median **78** -- assistants out-ranking head coaches -- and AC's max (99) matched HC's max, despite a real assistant's $800K salary ceiling being a fraction of a real head coach's $4M salary floor.

**The fix:** `app/models/coach.py` adds `REPUTATION_TIER_BAND` (HC 84-99, COORD 55-76, AC 35-58) and `reputation_from_tier_percentile()` -- each tier's real salary percentile (unchanged calculation) now maps onto THAT tier's own band instead of one shared band. The bands are each tier's real floor/ceiling salary (S.2's HC $4-10M / OC+DC $1-2.5M / ST $0.7-1.5M / AC $0.2-0.8M) evaluated on a log scale (compensation is log-normal; a linear anchor collapses HC's range to a handful of points) against the full real 2026 range ($200K -> $10M) mapped onto [35, 99]. HC and COORD never overlap (their real dollar ranges don't either); COORD and AC overlap by 3 points (55-58) -- a real top-paid assistant can plausibly out-earn a real bottom-tier ST coordinator, so a light boundary overlap is the accurate outcome, not a bug. `scripts/import_coaches.py::reputation_from_salary()` now takes the tier and forwards to this shared function (fresh saves/template rebuilds).

**One-time rescale of existing coaches:** `app/core/db.py::_migrate_schema()`'s `reputation_retiered` block (column-existence guard, identical idempotency pattern to `legacy_salary_rescaled`) re-scores every real-seeded coach (`pool_tier IS NULL`, i.e. not R3d's Tier 3 candidate pool, which already draws from its own independent bands) using their real, unchanged role and salary. Deliberately NOT a random re-roll: each coach's six performance ratings are shifted by the SAME delta their reputation moves by, clamped to 0-99, preserving that coach's own relative profile (a strong developer with weak discipline stays that shape) instead of becoming an unrecognizable new coach.
...
## Appendix V. September 20, 2026 Playtest Batch (implemented)

Brian's second playtest pass (22 items, "To do.docx" plus mid-session follow-ups), triaged bugs-first per this project's standing priority order. This appendix covers the items landed the same session; the rest stay open, tracked in ROADMAP.md rather than duplicated here.

### V.1 Real sudden-death overtime (supersedes Sec 6.3's undocumented gap, playoffs.py's own disclosed simplification)

Brian's report: "There can not be ties in the playoffs" plus a real screenshot showing a Divisional-round game finish 17-17 and a Wild Card game finish 20-20, with the bracket silently advancing whichever side the engine's tiebreak defaulted to. Root cause, confirmed live: **no overtime simulation existed anywhere in this engine**, regular season or playoffs -- `game_sim.simulate_game()` just stopped after a fixed regulation drive count and `winner = "home" if h >= a else "away"` silently picked a side even on a dead-even score. This was disclosed (`playoffs.py`'s own module docstring, ROADMAP.md's ties entry) but never actually built.

**The fix:** `app/engine/game_sim.py` gains `_simulate_overtime_period()`, a real sudden-death OT period matching the CURRENT NFL rule (not the pre-2010 "any score wins" version): the first score of the period wins outright, UNLESS it's a bare field goal, in which case the other team gets exactly one guaranteed answering possession -- they win with a TD, re-tie with a matching FG (then it's pure next-score-wins for the rest of the period), or lose outright by failing to score at all. This engine has no clock (`drive_sim.py`'s own long-standing limitation), so "one period" is approximated as a bounded drive count (`max(4, drives_total // 4)`) rather than a real 10/15-real-minute window -- generous enough that it only ever runs out in a genuine defensive slog.

`simulate_game()` gained a `playoff: bool` flag: **both** regular-season and playoff games get at least one real OT period when tied after regulation (real NFL rule). Regular season stops there -- a real NFL regular-season game CAN still end tied after one OT period, and eliminating that entirely would have overcorrected past real NFL behavior, not fixed a bug. Playoff games instead keep playing additional full periods (`OT_MAX_PLAYOFF_PERIODS = 10`, a safety valve against a pathological RNG run, not a real rule) until the tie breaks -- a playoff game is never allowed to end tied, full stop. `app/services/season_state.py`'s playoff call site (`simulate_playoff_round`) passes `playoff=True`; the regular-season and preseason call sites are unchanged (default `False`).

**Side effects, both real fixes not separate work:** `playoffs.py`'s `Matchup.winner_abbr` already reads `result.winner` -- once OT actually resolves close playoff games for real instead of defaulting to home, the bracket now shows the team that actually won, which was almost certainly the source of Brian's separate "#1 seed not showing in the bracket" report from the same screenshot batch (a lower seed was shown advancing past a tied game against the #1 seed under the old silent-tiebreak behavior). `headlines.py`'s `"playoff_tie"` template and its `if h == a` trigger in `season_state.py` are kept as a safety-net fallback for the vanishingly rare `OT_MAX_PLAYOFF_PERIODS`-exhausted case, not removed -- their comments/docstrings updated to say so rather than claim no OT model exists.

**Not done here, disclosed:** the regular-season tie RATE itself (independent of the playoff hard-rule fix) is now real but untuned -- no target tie-rate or calibration pass was run against `tests/test_stat_realism.py`'s existing bands. A real OT period should measurably reduce ties versus the old zero-OT baseline; whether it lands in a realistic real-NFL range (~0.1-0.3% of games) is a follow-up measurement, not verified in this pass.

### V.2 Assistant coaches missing Running Backs / Linebackers focus options

Brian's report: "Assistants should have all position groups to choose from as a focus. Currently there are no LB or running backs." Confirmed: `app/models/coach.py`'s per-role focus menus (Sec 4/5 above) had a narrow, single-position-group option for every group EXCEPT RB and LB -- QB, Receivers (WR/TE), OL, DL, and Secondary each had one, but touching RB or LB meant a broader combo (Running Game, Run Defense, Pass Defense, QB Pressure) only. **The fix:** new `FOCUS_RUNNING_BACKS`/`FOCUS_LINEBACKERS` options, added to OC's and AC's menus (`FOCUS_OPTIONS_BY_ROLE`) and AC's AI-autonomy need-driven option pools (`app/services/coach_ai.py`'s `_OFFENSE_NEED_OPTIONS`/`_DEFENSE_NEED_OPTIONS`), each mapped 1:1 to `rb_coaching`/`lb_coaching` (`FOCUS_RATING_WEIGHTS`), targeting `["RB"]`/`["LB"]` (`FOCUS_POSITION_GROUPS`), at the same breadth multiplier as every other narrow single-group option (`FOCUS_BREADTH_MULTIPLIER`). Every menu is already template-driven (`focus_options_for()`), so no template changes were needed.

### V.3 Player position mislabeled "WR/TE" on the Awards page (Brian's mid-session flag)

"Player on the awards screen or anywhere else should not have WR/TE as their position. WR and TE are separate positions for everything but coach focus." Root cause: `AwardCandidate.position` is a STAT-POOL label ("QB"/"RB"/"WR/TE"/"DEF"), not a real individual position -- OPOY/MVP/ROY score every receiving-target player (WR, TE, or even a receiving back) in one shared "WR/TE" pool, and the Awards page template rendered that pool label directly as if it were the player's actual position. The frozen end-of-season path (`season_honors.py`'s `finalize_regular_season()`) already correctly resolved each candidate's real position into a `player_position` field before saving; the LIVE, in-season Awards Race never did, so the bug only showed up mid-season.

**The fix:** `AwardCandidate` gains the same `player_position` field at the dataclass level (default `""`, so the template's existing `c.player_position or c.position` fallback is unaffected for historical/pure candidates with no live DB access). `awards.py`'s `_offensive_candidates()`/`_defensive_candidates()` (the live, DB-backed wrappers) now resolve every candidate's real `Player.position` via a new shared `_with_real_positions()` helper, same lookup shape as `season_honors.py`'s. `mvp_from_candidates()` (the MVP blend, which used to rebuild `AwardCandidate` objects and silently drop this field) now forwards it. Confirmed live via a real simulated season: a receiving running back correctly shows `player_position="HB"`, not "WR/TE" -- the pool a candidate scores within and the position displayed for them are no longer conflated. Not extended to the Hall of Fame page's own separate `HOFCandidate` (`history_store.py`), which has the identical "WR/TE" label for career pools -- out of scope here (many HOF candidates are long-retired with no live `Player` row to resolve against, a real architectural difference from the Awards page's always-live candidates), flagged as a disclosed follow-up if Brian wants it too.

### V.4 Free agent signed straight to the practice squad; counter-offer named a non-tradeable player

Two real, unrelated regressions found via the same playtest report and a follow-up: "After signing a free agent he went to my practice squad not my roster... his salary didn't count against my salary cap" and "The counter offer asked for a player not on my team."

**Free agency:** `app/engine/free_agency.py`'s `release_expired_contracts()` clears a player's `team_abbr` on contract expiry but never reset `roster_status` back to `ACTIVE` -- unlike the OTHER two release paths (`main.py`'s `roster_release_player`, `roster_prep.py`'s cut-to-53 overflow release), which both already do. A free agent whose LAST team had him on PS/IR before his contract expired carried that stale status into the free-agent pool; `main.py`'s `/free-agency/offer` ACCEPT branch never set it either, so signing him landed him straight on the practice squad even with real roster room. Both fixed: the ACCEPT branch now sets `roster_status = ACTIVE` on signing, and `release_expired_contracts()` resets it too. The separate "salary didn't count against the cap" half of the report turned out to be a false alarm on investigation -- `contracts.team_cap_space()` already sums EVERY roster player's salary regardless of `roster_status` (PS/IR salaries count against the cap by design, confirmed via every call site), so nothing was actually missing; the player's salary was correctly counted the whole time, just badly explained by his incorrect PS placement.

**Trade counter-offer:** `main.py`'s `gm_desk_trade_counter` route queried the user's FULL roster (`team_abbr == user_abbr`, no status filter) as the counter-offer's candidate pool, but the visible give-side roster grid and strict trade validation (`_trade_side_context`, R16 Sec 4.1/decision #15) both exclude `PRACTICE_SQUAD`/`ELEVATED` players as non-tradeable. An R16 commit (`16686a61`) added that exclusion to the visible grid and validation but never updated the counter-offer's own candidate query to match, so `trades.build_counter_offer()` could still select one of those players as "the addition that closes the deal" -- a real player on the user's roster, but one they could never actually add through the trade UI. Fixed by applying the identical `roster_status not in (PRACTICE_SQUAD, ELEVATED)` filter to the counter-offer's candidate pool.

### V.5 Practice squad is young and developmental (two passes)

Brian's report: "The practice squad shouldn't be filled with players who are old with high OVRs. It should be players who will sign for the league minimum... mostly younger players fighting for a spot... Maybe there needs to be a gate based on the spread between ovr and pot."

**Pass 1 (2026-09-21):** `free_agency.fill_practice_squad_gaps()` picked purely by highest current OVR. It now gates on real remaining upside (`potential - overall_rating >= PS_MIN_UPSIDE`, 5) and ranks by potential, youngest first as the tiebreak. **Pass 1 shipped with a fallback** ("if nobody clears the gate, use the whole pool") and it did not work on a fresh save: a new save's entire free-agent pool is only ~41 real veterans (Bobby Wagner, Tyreek Hill, Kevin Zeitler...), **none** with 5+ upside, so the fallback fired every time and reproduced the old behavior exactly. Brian caught it from a Render screenshot.

**Pass 2 (2026-09-23):** the fallback is gone -- an open slot beats an established veteran. And per Brian's direction ("create a pool deep enough for all teams to have 4-5 PS players... randomly generated to fall within our specifications for PS players"), `roster_prep.ensure_practice_squad_prospect_depth()` guarantees `len(TEAMS) x PS_PROSPECTS_PER_TEAM` (32 x 5 = 160) PS-*eligible* free agents at all times, generated by `draft.practice_squad_prospect_player()`: same pipeline/ID scheme as the existing filler rookies (age 22-24, OVR 45-62, league-minimum 1-year deal) but with potential lifted to at least `PS_MIN_UPSIDE` above overall (the existing filler rookies only pass the gate ~60% of the time), spread across positions in proportion to `ROSTER_REQUIREMENTS` (no K/P). Deterministic and idempotent. It runs before AI teams' PS fill, again at the end of `prepare_ai_rosters()` (so the user's own pool isn't drained), and at the start of the user's Auto-Fill PS click.

**Interpretation to confirm with Brian:** an AI team's automatic top-up now stops at `PS_PROSPECTS_PER_TEAM` (5) prospects (`fill_practice_squad_gaps(target_size=...)`), not the full 16, so the first teams in line can't drain the shared pool; cut-overflow veterans a team already carries (`auto_cut_team_to_limits`) are extra. The user's own Auto-Fill PS still fills to 16. **Known limit:** the PS signing is still cap-gated (decisions #3/#12), so an over-cap AI team can't add bodies -- "every team has 4-5" holds for teams with cap room. Existing veterans already sitting on a saved practice squad are untouched; only future fills change.

### V.6 Dashboard: playoff box score and bye week

**Box score:** the Dashboard's Box Score widget scanned only `season.schedule`, and `season.current_week` never advances during the playoffs, so it kept showing the last *regular-season* game through every playoff round. `main.py`'s `_last_played_game_for()` now checks the playoff bracket first (labeled by round, linking to `/playoffs/game/...`). The Scouting "Next Opponent" box was investigated and already handled the bracket correctly (`scouting.find_next_opponent`) -- the reported "No opponent confirmed yet" was a genuine bye/eliminated state.

**Bye week:** `find_next_opponent()` correctly skips a bye, but the real next game can be a week or more away, and showing it with no framing read as this week's game. The dashboard now detects the user's bye (regular season, not preseason/playoffs) and says "Bye week -- no game this week. Up next:" above the scouting content.

### V.7 GM Desk gets the Free Agents box

Roster's Top Free Agents box is now a shared macro (`macros.html`'s `free_agents_box`) fed by one shared helper (`main.py`'s `_free_agents_box_context()`), rendered on both Roster and GM Desk (GM Desk gained `fa_pos`/`fa_status` query params). One implementation, not a copy.

### V.8 Expiring contracts don't count against the cap during the re-sign stage
