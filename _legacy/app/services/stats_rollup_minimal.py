...
def rollup_game_stats_minimal(s: Session, game_id: int) -> bool:
    """Minimal rollup stats for a specific game from PBP data"""
    ...
    try:
        ...
        s.commit()
        logger.info(f"Successfully rolled up minimal stats for game {game_id}")
        return True
        
    except Exception as e:
        logger.error(f"Error rolling up minimal stats for game {game_id}: {e}")
        s.rollback()
        return False

def rollup_season_stats_minimal(s: Session, season: int) -> bool:
    """Minimal rollup stats for all games in a season"""
    ...
    try:
        ...
        for game_id, game_event_list in game_events.items():
            try:
                if rollup_game_stats_minimal(s, game_id):
                    success_count += 1
            except Exception as e:
                logger.error(f"Error rolling up minimal stats for game {game_id}: {e}")
                continue
        
        logger.info(f"Successfully rolled up minimal stats for {success_count}/{len(game_events)} games in season {season}")
        return success_count > 0
        
    except Exception as e:
        logger.error(f"Error rolling up minimal stats for season {season}: {e}")
        return False
