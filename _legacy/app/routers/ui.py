...
def playoffs_page(request: Request):
    return request.app.state.templates.TemplateResponse("playoffs.html", {"request": request, "title":"Playoffs", "season": 2025})

@router.get("/teams", response_class=HTMLResponse)
def teams_page(request: Request, session: Session = Depends(get_db)):
...
def games_page(season: int, week: int, request: Request, session: Session = Depends(get_db)):
...
def box_page(game_id: int, request: Request, session: Session = Depends(get_db)):
    teams = {t.id:t for t in session.exec(select(Team)).all()}
...
def health_ui(request: Request, session: Session = Depends(get_db)):
