        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {data.map((player) => (
              <div
                key={player.rank}
                className="flex items-center justify-between py-3 px-3 hover:bg-[#2d4a6f]/30 rounded transition-colors cursor-pointer min-h-[44px]"
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <span className="text-[#d4af37] w-6 flex-shrink-0">{player.rank}</span>
                  <div className="min-w-0 flex-1">
                    <div className="text-white truncate">{player.player_name}</div>
                    <div className="text-[#94a3b8] text-xs">
                      {player.team_id} · {player.position}
                    </div>
                  </div>
                </div>
                <div className="text-white ml-3 flex-shrink-0">
