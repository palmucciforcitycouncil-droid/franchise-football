#!/usr/bin/env python3
"""
NFL Simulation Engine - Phase 4: Balanced Drive Termination
"""
...
def run_phase4_calibrated_season(session: Session, weeks: int = 4, seed: int = 2025):
    """Run Phase 4 calibrated mini season with balanced drive termination."""
    random.seed(seed)
...
def _generate_phase4_pbp_events(session: Session, game_id: int, home_team_id: int, away_team_id: int, seed: int):
    """Generate Phase 4 PBP events with balanced drive termination."""
    random.seed(seed)
    ...
    for drive_num in range(num_drives):
        ...
        distance = 10
        drive_plays = 0
        max_drive_plays = random.randint(6, 12)  # Balanced drive length
        
        # Drive progression with balanced termination logic
        while drive_plays < max_drive_plays and current_yardline < 100:
            # Determine play type based on down/distance and field position
            play_type = _determine_phase4_play_type(down, distance, current_yardline)
            
            if play_type == "pass":
                yards_gained, completed, interception, sack = _generate_phase4_pass_play(down, distance)
                ...
                yards_gained = _generate_phase4_run_play()
...
def _determine_phase4_play_type(down: int, distance: int, yardline: int) -> str:
    """Determine play type with balanced drive termination logic."""
    # Modern NFL is pass-heavy
    pass_prob = 0.65  # Base 65% pass rate
    
    # BALANCED FIX: Moderate 3rd down failure rates
    if down == 3:
        if distance <= 2:
            ...
            pass_prob = 0.80  # Long yardage favors pass (reduced from 85%)
    
    # BALANCED FIX: Implement balanced 4th down logic
    elif down == 4:
...
def _generate_phase4_pass_play(down: int, distance: int):
    """Generate realistic passing play results with balanced sack rates."""
    # Modern NFL completion rate ~65-70%
    completed = random.random() < 0.68
    
    # BALANCED FIX: Moderate sack rates
    sack_prob = 0.06  # Base 6% sack rate
    if down == 3 and distance >= 7:
        sack_prob = 0.09  # Slightly higher sack rate on 3rd & long (reduced from 12%)
    elif down == 2 and distance >= 10:
        sack_prob = 0.08  # Higher sack rate on 2nd & long (reduced from 9%)
...
def run_phase4_calibration_report():
    """Run Phase 4 calibration and provide detailed report."""
    print("NFL SIMULATION ENGINE - PHASE 4 CALIBRATION REPORT")
    print("=" * 60)
    
    with memory_db() as session:
        print("Running Phase 4 calibrated 4-week mini season...")
        out = run_phase4_calibrated_season(session, weeks=4, seed=2025)
