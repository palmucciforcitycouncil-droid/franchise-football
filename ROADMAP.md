...
5. **Dashboard page still doesn't match the Figma 3-column grid layout** — missed in the original chunk list, added as M9 (2026-09-09). Ready for a fresh session now, no blockers.
...
| **M9** | Dashboard page redesign | **Missed in the original chunk list — added 2026-09-09 after Brian's own live testing surfaced it via a screenshot, in a second concurrent session.** Current `dashboard.html` is a single-column stack of cards (Weekly Gameplan, Scouting, etc.). Real spec is `docs/figma-export/src/app/DASHBOARD_DOCUMENTATION.md` (lines ~43-59): a 3-column CSS Grid — Row 1: Division Standings \| Team Schedule \| Scouting Panel (3 equal columns); Rows 2-3: left column has Power Rankings/Top Performers stacked, right two columns hold Box Score & Play-by-Play. This is a layout/CSS-Grid rebuild of an existing, already-real page — no new backend data needed, every widget already exists, just needs to move into the grid. | `app/templates/dashboard.html`, `app/templates/base.html` (new grid CSS) | M | **Haiku** or Sonnet (mechanical, same category as M3/M4) | own session |

**Total remaining MVP: M8 + M9.** M8 is blocked on the decision in §3; M9 is unblocked and ready for a fresh session right now. M1-M7 are done.
