...
3. Two UI pages that still don't match the Figma redesign: Roster (view toggle, quotas, embedded depth chart), HOF (richer layout + League Record Book/Super Bowl History addition from the v4.4/4.5 GDD rewrite). ~~Stats (customizable columns)~~ — **done (M3, 2026-09-09)**, see §2's M3 row and HANDOFF.md item 39.
...
**Total remaining MVP: roughly 2-4 focused sessions**, most of them small-to-medium, now that M1 (the one chunk with real design risk), M2, and M3 are done.
