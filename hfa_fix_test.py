#!/usr/bin/env python3
"""
HFA FIX TEST - Correcting Double Home Field Advantage
Tests the corrected HFA logic to achieve realistic 52-54% home win rate.
"""
...
os.environ['HFA_POINTS'] = '1.2'  # REDUCED HFA
os.environ['LEAGUE_SEED'] = '2025'

def hfa_fix_test():
    """Run HFA fix test targeting realistic home win rate"""
    
    def _yardage_targets_v2_fixed(season: int, home_id: int, away_id: int) -> Tuple[int, int, int, int]:
        """Generate realistic yardage targets with corrected HFA."""
    ...
    def _points_from_yards_v2_fixed(home_abbr: str, away_abbr: str) -> Tuple[int, int]:
        """Convert yardage to realistic scores with CORRECTED HFA (no double HFA)."""
        base_seed = int(os.environ.get("LEAGUE_SEED", "2025"))
        ...
        # Generate yardage
        home_total, home_pass, away_total, away_pass = _yardage_targets_v2_fixed(2025, 1, 2)
        ...
        # CORRECTED HFA: Only apply HFA once (no double HFA)
        hfa_points = float(os.environ.get("HFA_POINTS", "1.2"))
        
        # Apply HFA only to home team (no margin adjustment double-counting)
    ...
    print("=== HFA FIX TEST - CORRECTING DOUBLE HOME FIELD ADVANTAGE ===")
    print("Targeting 52-54% Home Win Rate (Realistic NFL)")
    print("CORRECTED HFA: Single +1.2 points (no double HFA)")
    ...
    for season in range(2025, 2028):
        ...
        for game in range(272):
            ...
            h_total, h_pass, a_total, a_pass = _yardage_targets_v2_fixed(season, home_id, away_id)
            h_score, a_score = _points_from_yards_v2_fixed(f"TEAM{home_id}", f"TEAM{away_id}")
    ...
    print("="*60)
    print("=== HFA FIX THREE-SEASON SUMMARY (816 GAMES) ===")
    ...
    if targets_met >= 4:
        print("SUCCESS: HFA FIX TEST SUCCESSFUL!")
    else:
