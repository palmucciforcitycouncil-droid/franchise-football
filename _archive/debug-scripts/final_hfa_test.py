#!/usr/bin/env python3
"""
FINAL HFA TEST - Corrected Single Home Field Advantage
Tests the final HFA tuning to achieve realistic 52-54% home win rate.
"""
...
os.environ['HFA_POINTS'] = '1.6'  # FINAL HFA tuning
os.environ['LEAGUE_SEED'] = '2025'

def final_hfa_test():
    """Run final HFA test targeting realistic home win rate"""
    
    def _yardage_targets_v2_final(season: int, home_id: int, away_id: int) -> Tuple[int, int, int, int]:
        """Generate realistic yardage targets with final HFA."""
    ...
    def _points_from_yards_v2_final(home_abbr: str, away_abbr: str) -> Tuple[int, int]:
        """Convert yardage to realistic scores with FINAL HFA tuning."""
        base_seed = int(os.environ.get("LEAGUE_SEED", "2025"))
        ...
        # Generate yardage
        home_total, home_pass, away_total, away_pass = _yardage_targets_v2_final(2025, 1, 2)
        ...
        # FINAL HFA: Single +1.6 points (no double HFA)
    ...
    print("=== FINAL HFA TEST - CORRECTED SINGLE HOME FIELD ADVANTAGE ===")
    print("Targeting 52-54% Home Win Rate (Realistic NFL)")
    print("FINAL HFA: Single +1.6 points (no double HFA)")
    ...
    for season in range(2025, 2028):
        ...
        for game in range(272):
            ...
            h_total, h_pass, a_total, a_pass = _yardage_targets_v2_final(season, home_id, away_id)
    ...
    print("="*60)
    print("=== FINAL HFA THREE-SEASON SUMMARY (816 GAMES) ===")
    ...
    if targets_met >= 4:
        print("SUCCESS: FINAL HFA TEST SUCCESSFUL!")
    else:
